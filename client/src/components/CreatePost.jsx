    ...
    if (localStorage.getItem("token")) { // If user logged in return the post wysiwyg editor
        return (
            ...
            >
                <Box
                    component="form" sx={{
                        display: "flex", flexDirection: "column", alignItems: "center", width: { xs: "90%", sm: "60%" }, border: "2px solid black", marginTop: "20px",
                    }}
                >
                    <Typography sx={{ fontSize: "32px", margin: "10px" }}>Create a post</Typography>
                    <TextField inputProps={{ maxLength: 60 }} sx={{ margin: "10px", width: { xs: "90%", sm: "60%" } }} id="title outlined-basic" className="post-title-text" required label="Title" variant="outlined" onChange={(e) => setTitle(e.target.value)} />
                    <Box sx={{ width: "100%" }}>
                        <Editor
