    ...
    if (post.solved === true) { // Lists solved posts
        return (
            ...
            >
                ...
                <Box sx={{ fontSize: "16px" }} multiline="true" dangerouslySetInnerHTML={{ __html: DOMPurify.sanitize(draftToHtml(JSON.parse(post.content))) }} />
            </Paper>
        );
    ...
    if (user) { // Returns user information and posts and returns only the user if the user has no posts yet
        if (posts) {
            return (
                ...
                >
                    <Box sx={{
                        width: { xs: "90%", sm: "60%" }, display: "flex", justifyContent: "center", alignItems: "center", flexDirection: "column",
                    }}
                    >
                        ...
                        >
                            ...
                            <Typography>
                                ...
                                {DateTime.fromJSDate(new Date(user.createdAt)).toLocaleString(DateTime.DATETIME_MED)}
                            </Typography>
                            <Typography>
                                Bio:
                                {user.bio}
