    ...
    if (post.solved !== true) {
        return (
            <Box sx={{ display: "flex", flexDirection: "column" }}>
                ...
                <TextField id="outlined-basic" className="comment-text" label="Comment" variant="outlined" onChange={(e) => setNewComment(e.target.value)} />
...
function EditCmt({
    userId, cAuthor, commentId, navigate,
}) {
    const token = userId;
...
function PostInfo({ post, navigate }) {
    ...
    if (post.createdAt >= post.updatedAt) { // Returns different versions depending on if the post has been edited or not
        return (
            <Paper sx={{
                backgroundColor: "#dbdbdb", width: { xs: "90%", sm: "60%" }, margin: "24px", overflowWrap: "break-word", border: "1px solid black", padding: "10px",
            }}
            >
                <Typography multiline="true" sx={{ fontSize: "24px", textDecoration: "underline" }}>{post.title}</Typography>
                <Box multiline="true" dangerouslySetInnerHTML={{ __html: DOMPurify.sanitize(draftToHtml(JSON.parse(post.content))) }} />
                <Typography sx={{ display: "inline" }}>Posted </Typography>
                <Typography sx={{ fontWeight: "bold", cursor: "grab", display: "inline" }} onClick={goToProfile}>
                    @
                    {post.owner.username}
                </Typography>
                <Typography>
                ...
                <Box>
                    <EditBtn post={post} navigate={navigate} />
                    <SolveBtn post={post} navigate={navigate} />
                </Box>
            </Paper>
        );
    }
    if (post.solved === true) { // Checks if post is solved or not, since marking post as solve is always edit to the data, it is always edited so no need to check
        return ( // this in the created if-clause above this
            <Paper sx={{
                backgroundColor: "lightGreen", width: { xs: "90%", sm: "60%" }, margin: "24px", overflowWrap: "break-word", border: "1px solid black", padding: "10px",
            }}
            >
    ...
    } // Returns this if post is not solved so it has gray background
    return (
        <Paper sx={{
            backgroundColor: "#dbdbdb", width: { xs: "90%", sm: "60%" }, margin: "24px", overflowWrap: "break-word", border: "1px solid black", padding: "10px",
        }}
...
function Comment({ comment }) { // Different returns if the comment has been edited or not
    ...
    if (comment.createdAt >= comment.updatedAt) { // Returns different versions depending on if the comment has been edited or not
        return (
            <Paper sx={{
                margin: "10px", backgroundColor: "#dbdbdb", border: "1px solid black", padding: "10px",
            }}
            >
...
function ViewPost() {
        ...
        if (localStorage.getItem("token")) { // if user is logged in return post and comments with the ability to post comments and edit post if the user owns it
            return (
                ...
                >
                    ...
                    <SendComment newComment={newComment} setNewComment={setNewComment} post={post} navigate={navigate} />
                    <Box sx={{
                        display: "flex", flexDirection: "column", width: { xs: "90%", sm: "60%" }, alignItems: "center",
                    }}
                    >
                        ...
                        {comments.map((comment) => (
                            <Box sx={{ fontSize: "24px", width: "100%", border: "1px solid black" }} key={comment._id}>
                                ...
                                <EditCmt userId={localStorage.getItem("token")} cAuthor={comment.author} commentId={comment._id} navigate={navigate} />
                            </Box>
                        ))}
                    </Box>
                </Box>
            );
        } // if no user is logged in just return post and comments
        return (
            <Box sx={{
                display: "flex", flexDirection: "column", width: "100%", alignItems: "center",
            }}
    ...
    } // when no posts or comments are loaded
