...
function PostOverview ({post, navigate}) {
    ...
    if(post.createdAt >= post.updatedAt) { //if post has been created and not edited return this
        return(
            <Paper sx={{display: "flex", flexDirection: "column", justifyContent: "center", padding: "10px", cursor:'grab', backgroundColor: "lightgray", margin: "5px", width: {xs: "90%", sm: "60%"}, height: "fit-content", "&:hover": {transition: "all 0.5s",  backgroundColor: "gray"}, border: "solid 1px black", wordBreak: "break-word"}} onClick={() => handleClick(post._id)} key={post._id}>
        ...
        if(post.solved === true) { //If post has been solved return green background
            return(
                <Paper sx={{display: "flex", flexDirection: "column", justifyContent: "center", padding: "10px", cursor:'grab', backgroundColor: "lightGreen", margin: "5px", width: {xs: "90%", sm: "60%"}, height: "fit-content", "&:hover": {transition: "all 0.5s",  backgroundColor: "green"}, border: "solid 1px black", wordBreak: "break-word"}} onClick={() => handleClick(post._id)} key={post._id}>
                    <Typography>{post.title} | @{post.owner.username}</Typography>
