...
function PostOverview({ post, navigate }) {
    ...
    if (post.createdAt >= post.updatedAt) { // if post has been created and not edited return this
        return (
            <Paper
                sx={{
                    display: "flex", flexDirection: "column", justifyContent: "center", padding: "10px", cursor: "grab", backgroundColor: "lightgray", margin: "5px", width: { xs: "90%", sm: "60%" }, height: "fit-content", "&:hover": { transition: "all 0.5s", backgroundColor: "gray" }, border: "solid 1px black", wordBreak: "break-word",
                }} onClick={() => handleClick(post._id)} key={post._id}
            >
                <Typography>
                    {post.title}
                    {" "}
    ...
    } // if post has been edited after creating return this
    if (post.solved === true) { // If post has been solved return green background
        return (
            <Paper
                sx={{
                    display: "flex", flexDirection: "column", justifyContent: "center", padding: "10px", cursor: "grab", backgroundColor: "lightGreen", margin: "5px", width: { xs: "90%", sm: "60%" }, height: "fit-content", "&:hover": { transition: "all 0.5s", backgroundColor: "green" }, border: "solid 1px black", wordBreak: "break-word",
                }} onClick={() => handleClick(post._id)} key={post._id}
            >
                <Typography>
                    {post.title}
    ...
    } // If post has not been solved return gray background
    return (
        <Paper
            sx={{
                display: "flex", flexDirection: "column", justifyContent: "center", padding: "10px", cursor: "grab", backgroundColor: "lightgray", margin: "5px", width: { xs: "90%", sm: "60%" }, height: "fit-content", "&:hover": { transition: "all 0.5s", backgroundColor: "gray" }, border: "solid 1px black", wordBreak: "break-word",
...
function ListPost({ data, navigate }) { // Lists all the posts
    return (
        ...
        >
            {data.map((post) => <PostOverview key={post._id} post={post} navigate={navigate} />)}
        </Box>
    );
}
...
function Loggedin({ data, navigate }) { // Logged in version to show create post button
    return (
        <Box sx={{
            width: "100%", display: "flex", alignItems: "center", flexDirection: "column", marginTop: "10px",
        }}
        >
            <Box>
                <Box sx={{
...
function Posts() {
    ...
    if (localStorage.getItem("token")) {
        return (
            <Loggedin data={data} navigate={navigate} />
        );
    }
    return (
        <Notloggedin data={data} navigate={navigate} />
    );
}
